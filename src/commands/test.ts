...
export async function testCommand(): Promise<void> {
  ...
  console.log(chalk.gray(`Facilitator balance: ${facilitatorBalance / 1e9} SOL`));

  if (payerBalance < 0.1 * 1e9) {
    throw new Error('Payer wallet has insufficient SOL. Please run `xc-setup fund` first.');
  }
  ...
  if (testTokenMint) {
    try {
      ...
      const tokenAccount = await getAccount(connection, payerTokenAddress);
      const tokenBalance = Number(tokenAccount.amount) / 10 ** 9;
      console.log(chalk.green(`\n✅ Token balance verified: ${tokenBalance} tokens`));
    } catch (error) {
      console.log(chalk.yellow(`⚠️  Could not verify token balance (token account may not exist)`));
    }
  }
