...
const AIRDROP_AMOUNT_SOL = 2; // 2 SOL for fees
const TEST_TOKEN_AMOUNT = 1000; // 1000 test tokens
...
export async function fundCommand(): Promise<void> {
  ...
  if (!await fs.pathExists(envPath)) {
    throw new Error('No .env file found. Please run `xc-setup init` first.');
  }
  ...
  const payerKeypairPath = envVars.PAYER_KEYPAIR_PATH || path.join('keys', 'payer.json');
  const facilitatorKeypairPath = envVars.FACILITATOR_KEYPAIR_PATH || path.join('keys', 'facilitator.json');

  if (!await fs.pathExists(payerKeypairPath) || !await fs.pathExists(facilitatorKeypairPath)) {
    throw new Error('Keypair files not found. Please run `xc-setup init` first.');
  }
  ...
  console.log(chalk.blue(`💸 Airdropping ${AIRDROP_AMOUNT_SOL} SOL to Payer...`));
  ...
  console.log(chalk.green(`✅ Payer funded: ${payerKeypair.publicKey.toBase58()}`));
  console.log(chalk.gray(`   Tx: ${payerSignature}`));

  // Airdrop SOL to facilitator
  console.log(chalk.blue(`💸 Airdropping ${AIRDROP_AMOUNT_SOL} SOL to Facilitator...`));
  ...
  console.log(chalk.green(`✅ Facilitator funded: ${facilitatorKeypair.publicKey.toBase58()}`));
  ...
  console.log(chalk.blue(`💸 Minting ${TEST_TOKEN_AMOUNT} test tokens to Payer...`));
